...
import { LogOut as LogOutIcon, Search, Check, X, Plus, Eye, AlertTriangle, Pencil, Undo2 } from 'lucide-react';
...
const AdminEarlyOut = () => {
  ...
  const fetchData = useCallback(async () => {
    try { setLoading(true);
      const [reqRes, empRes] = await Promise.all([
        axios.get(`${API}/early-out-requests`, { headers: getAuthHeaders() }),
  ...
  const handleApprove = async () => {
    ...
    try {
      await axios.put(`${API}/early-out-requests/${selected.id}/approve`, { is_lop: lopChoice === 'lop', lop_remark: lopRemark || null }, { headers: getAuthHeaders() });
  ...
  const handleReject = async () => {
    if (!selected) return;
    setActionLoading(true);
    try { await axios.put(`${API}/early-out-requests/${selected.id}/reject`, {}, { headers: getAuthHeaders() }); toast.success('Rejected'); setShowReject(false); setShowDetail(false); fetchData(); }
    catch (e) { toast.error(e.response?.data?.detail || 'Failed'); }
    finally { setActionLoading(false); }
  };
  ...
  const handleReset = async (r) => {
    if (!r) return;
    const ok = window.confirm(`Reset this ${r.status} early-out request for ${r.emp_name} back to Pending?`);
    if (!ok) return;
    try {
      await axios.post(`${API}/early-out-requests/${r.id}/reset`, { reason: 'manual reset' }, { headers: getAuthHeaders() });
            ...
            <tr key={r.id}>
              ...
              {isHR && <td>
                <div className="flex gap-1">
                  <Button size="sm" variant="outline" onClick={() => { setSelected(r); setShowDetail(true); }} className="rounded-lg h-8 px-2" data-testid={`view-eo-${r.id}`}><Eye className="w-3 h-3" /></Button>
                  <Button size="sm" variant="outline" onClick={() => openEdit(r)} className="rounded-lg h-8 px-2 border-blue-300 text-blue-700 hover:bg-blue-50" data-testid={`edit-eo-${r.id}`} title="Edit"><Pencil className="w-3 h-3" /></Button>
                  {isPending && <>
                    <Button size="sm" onClick={() => { setSelected(r); setLopChoice('no_lop'); setLopRemark(''); setShowApprove(true); }} className="bg-emerald-500 hover:bg-emerald-600 text-white h-8 px-2 rounded-lg" data-testid={`approve-eo-${r.id}`}><Check className="w-3 h-3" /></Button>
                    <Button size="sm" onClick={() => { setSelected(r); setShowReject(true); }} className="bg-red-500 hover:bg-red-600 text-white h-8 px-2 rounded-lg" data-testid={`reject-eo-${r.id}`}><X className="w-3 h-3" /></Button>
                  </>}
                  {!isPending && (
                    <Button size="sm" variant="outline" onClick={() => handleReset(r)} className="rounded-lg h-8 px-2 border-amber-300 text-amber-700 hover:bg-amber-50" data-testid={`reset-eo-${r.id}`} title="Reset to Pending"><Undo2 className="w-3 h-3" /></Button>
                  )}
                </div>
              </td>}
            </tr>
    ...
    <div className="space-y-6 animate-fade-in" data-testid="admin-early-out-page">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-[#063c88] flex items-center justify-center"><LogOutIcon className="w-5 h-5 text-white" /></div>
          <div><h1 className="text-2xl font-bold text-slate-900" style={{ fontFamily: 'Outfit' }}>Early Outs</h1><p className="text-sm text-slate-500">Manage employee early out requests</p></div>
        </div>
        {isHR && <Button onClick={() => setShowApply(true)} className="bg-[#063c88] hover:bg-[#052d66] text-white rounded-xl" data-testid="admin-apply-early-out-btn"><Plus className="w-4 h-4 mr-2" /> Apply for Employee</Button>}
      </div>

      <div className="card-flat p-4"><div className="max-w-sm"><EmployeeAutocomplete value={searchName} onChange={setSearchName} onSelect={(emp) => setSearchName(emp.full_name)} placeholder="Search employee..." data-testid="early-out-search" /></div></div>
      ...
      <Dialog open={showApprove} onOpenChange={setShowApprove}>
        <DialogContent className="bg-[#fffdf7] rounded-2xl"><DialogHeader><DialogTitle style={{ fontFamily: 'Outfit' }}><Check className="w-5 h-5 text-emerald-500 inline mr-2" />Approve Early Out</DialogTitle><DialogDescription>Choose LOP status</DialogDescription></DialogHeader>
      ...
      <Dialog open={showReject} onOpenChange={setShowReject}>
        <DialogContent className="bg-[#fffdf7] rounded-2xl"><DialogHeader><DialogTitle style={{ fontFamily: 'Outfit' }}><AlertTriangle className="w-5 h-5 text-red-500 inline mr-2" />Reject Early Out</DialogTitle><DialogDescription>Are you sure you want to reject?</DialogDescription></DialogHeader>
      ...
      <Dialog open={showApply} onOpenChange={setShowApply}>
        <DialogContent className="bg-[#fffdf7] rounded-2xl sm:max-w-lg"><DialogHeader><DialogTitle style={{ fontFamily: 'Outfit' }}>Apply Early Out for Employee</DialogTitle><DialogDescription>Submit on behalf of an employee</DialogDescription></DialogHeader>
      ...
      <Dialog open={showEdit} onOpenChange={setShowEdit}>
        <DialogContent className="bg-[#fffdf7] rounded-2xl sm:max-w-lg" data-testid="edit-eo-dialog">
          <DialogHeader>
            <DialogTitle style={{ fontFamily: 'Outfit' }} className="flex items-center gap-2">
              <Pencil className="w-5 h-5 text-[#063c88]" /> Edit Early Out Request
            </DialogTitle>
          ...
          {selected && (
            <div className="space-y-4 py-2">
              ...
              <div><Label>Date</Label><Input type="date" value={editForm.date} onChange={e => setEditForm({ ...editForm, date: e.target.value })} className="mt-1.5 rounded-lg" data-testid="edit-eo-date" /></div>
              <div className="grid grid-cols-2 gap-4">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleEdit} disabled={actionLoading} className="bg-[#063c88] hover:bg-[#052d66] text-white rounded-lg" data-testid="confirm-edit-eo">
