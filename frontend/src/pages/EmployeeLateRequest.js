...
import { Clock, Plus, Check, X, Edit2, AlertCircle } from 'lucide-react';
...
const EmployeeLateRequest = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${API}/late-requests`, { headers: getAuthHeaders() });
      setRequests(res.data);
    } catch { toast.error('Failed to load late requests'); }
  ...
  const handleSubmit = async () => {
    ...
    try {
      setFormLoading(true);
      if (editingId) {
        await axios.put(`${API}/late-requests/${editingId}`, form, { headers: getAuthHeaders() });
        toast.success('Late request updated');
      } else {
  ...
  return (
    <div className="space-y-6 animate-fade-in" data-testid="employee-late-request-page">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900" style={{ fontFamily: 'Outfit' }}>Late Requests</h1>
            <p className="text-sm text-slate-500">Apply and track late login requests</p>
          </div>
        </div>
        <Button onClick={() => { setEditingId(null); setForm({ date: '', expected_time: '', actual_time: '', reason: '' }); setShowDialog(true); }} className="bg-[#063c88] hover:bg-[#052d66] text-white rounded-xl" data-testid="apply-late-request-btn">
          <Plus className="w-4 h-4 mr-2" /> Apply Late Request
        </Button>
      </div>
              ...
              <tbody>
                {sortedRequests.length === 0 ? <tr><td colSpan="7" className="text-center py-12 text-slate-500">No late requests found</td></tr> : sortedRequests.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td>{r.status === 'pending' && <Button size="sm" variant="outline" onClick={() => handleEdit(r)} className="rounded-lg" data-testid={`edit-late-${r.id}`}><Edit2 className="w-3 h-3" /></Button>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="bg-[#fffdf7] rounded-2xl">
          ...
          <div className="space-y-4 py-4">
            ...
            <div><Label>Reason</Label><Textarea value={form.reason} onChange={e => setForm({ ...form, reason: e.target.value })} className="mt-1.5 rounded-lg min-h-[80px]" placeholder="Reason for late login..." data-testid="late-reason-input" /></div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowDialog(false)} className="rounded-lg">Cancel</Button>
            <Button onClick={handleSubmit} disabled={formLoading} className="bg-[#063c88] hover:bg-[#052d66] text-white rounded-lg" data-testid="submit-late-btn">{formLoading ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : 'Submit'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
...
export default EmployeeLateRequest;
