  ...
  return (
    <div className="space-y-6 p-6" data-testid="onboarding-completion-page">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">Onboarding Completion</h1>
          <p className="text-sm text-slate-500 mt-1">
            Track onboarding document completion. Automated reminder emails fire every 48 hours per employee
            until onboarding reaches 100%, then a one-time success email is sent.
          </p>
        </div>
      ...
      {/* Summary chips */}
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3" data-testid="oc-summary">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading && (
                <tr>
                  <td colSpan={7} className="px-4 py-10 text-center text-slate-400" data-testid="oc-loading">
              ...
              {!loading && filteredRows.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-4 py-10 text-center text-slate-400" data-testid="oc-empty">
              ...
              {!loading && filteredRows.map((row) => (
                <tr key={row.employee_id} className="hover:bg-slate-50/60" data-testid={`oc-row-${row.emp_id || row.employee_id}`}>
                  ...
                  <td className="px-4 py-3 min-w-[200px]">
                    <ProgressBar percent={row.onboarding_percent} />
                    <div className="text-xs text-slate-500 capitalize mt-1">{row.onboarding_status}</div>
