...
const stamp = (v) => (v ? String(v).slice(0, 16).replace('T', ' ') : '--');
...
const INFO_HEADERS = {
  ...
  N01: ['Leave Date', 'Leave Type', 'Duration', 'Applied On', 'Reason'],
...
const infoCells = (code, r, onReason) => {
  switch (code) {
    ...
    case 'P06':
      return [r.date, r.kind, r.work];
    case 'N01':
      return [r.date, dash(r.leave_type), dash(r.split), stamp(r.applied_at),
        <ReasonButton key="r" row={r} onOpen={onReason} />];
    case 'N02':
      return [r.date, dash(r.split), stamp(r.applied_at), r.reason];
    case 'N03':
...
const ReasonButton = ({ row, onOpen }) => {
  const has = !!(row.leave_reason || '').trim();
  return (
    <Button size="sm" variant="ghost" disabled={!has} title={has ? 'View leave reason' : 'No reason provided'}
      className="h-7 px-2" onClick={() => has && onOpen(row)} data-testid={`brsf-reason-${row.key}`}>
      <Eye className={`w-3.5 h-3.5 ${has ? 'text-slate-600' : 'text-slate-300'}`} />
    </Button>
  );
};
    ...
    <>
            ...
            if (['N07', 'N08'].includes(line.code)) {
              return (
                <tr key={r.id || i} className="border-t border-slate-200/70 text-slate-700" data-testid={`brsf-child-${line.code}-${i}`}>
                  {[dash(r.date), dash(r.time), dash(r.remarks), star(r.value), dash(r.created_by)].map((c, j) => (
      ...
      <Dialog open={!!reason} onOpenChange={(v) => !v && setReason(null)}>
        <DialogContent className="max-w-md" data-testid="brsf-reason-dialog">
          <DialogHeader>
            <DialogTitle>Leave Reason — {reason?.date}</DialogTitle>
            <DialogDescription>{reason?.leave_type} · {reason?.split}</DialogDescription>
          </DialogHeader>
